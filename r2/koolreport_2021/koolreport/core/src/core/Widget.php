...
class Widget
{
    ...
    /**
     * The cssClass attached to widget. EG working
     * 
     * @var string $themeHtmlType The cssClass attached to widget.
     */
    protected $themeHtmlType;
    ...
    public function __construct($params=null)
    {
        ...
        $this->themeHtmlType = Utility::get($this->params, "themeHtmlType");
        ...
        if ($theme) {
            ...
            if (!$this->themeCssClass) {
             ...
             if (!$this->themeHtmlType) {
                $this->themeHtmlType = Utility::get($theme->themeInfo(), "htmlType");
            }
    ...
    /**
     * Get the additional css class name EG working
     *      
     * @return string The base theme Html Type
     */
    protected function getThemeHtmlType()
    {
        return $this->themeHtmlType;
    }

    

    /**
     * Render this widget , updated by EG 13.3.2013
     * 
     * @return null
     */
    public function render()
    {
        ...
        if ($this->report->fireEvent("OnBeforeWidgetRender", $this)) {
            ...
            $id_rand=rand(1,999).rand(1,999).rand(1,999);
            //print_r($this->tmlType);
            //print_r($this);
            //echo "<krwidget widget-name='$this->name' widget-type='$type'" .($this->themeCssClass?" class='$this->themeCssClass'":""). '  '.($this->htmlType['type']?" input='".$this->htmlType['type']."'":"")."  >";
            //if(!(is_null($this->htmlType)))
            if (count($this->htmlType)>0){
                if ($this->cssClass['active']==1){
                    //echo $this->htmlType["type"].'sssss';
                //
                    if (trim($this->htmlType["type"])=='checkbox')
                    {
                        //echo '########';
                        echo '<label>'.$this->title .' </label>';
                        echo '<input id="'.$this->htmlType["type"].$id_rand .'" style="text-align:center" value="'.$this->value.'" type="'.$this->htmlType["type"] .'" />';
                    }
                    else{
                        //echo '$$$$$$$$';
                        echo '<input style="text-align:center" value="'.$this->value.'" type="'.$this->htmlType["type"] .'" />';
                    }
                }
                else{
                    echo '<label>'.$this->value .'</label>';
                }
                //echo $this->value;
                //$this->onRender();
            }
                ...
                //echo '#######@@!!!!!!!';
                echo "<krwidget widget-name='$this->name' widget-type='$type'" .($this->themeCssClass?" class='$this->themeCssClass'":"").">";
                $this->onRender();
                echo "</krwidget>";
            }
        }
