...
$draw = $_POST['draw'];
$row = $_POST['start'];
$rowperpage =1000000;// $_POST['length']; // Rows display per page
 // Ordering
...
//array_push($arr, 0);array_push($arr, 1);array_push($arr, 2); array_push($arr, 8);
 //$data[] = array()
for($tt=0;$tt<count($arr);$tt++)
{
    $i=$arr[$tt];
...
$sel = mysqli_query($con,"select count(*) as allcount from master_view");
...
## Response
$response = array(
    "draw" => intval($draw),"query"=>$empQuery,
